...
/*******************************************************************************
* Function Name: Clock_4kHz_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_4kHz_Start(void) 
{
    /* Set the bit to enable the clock. */
    Clock_4kHz_CLKEN |= Clock_4kHz_CLKEN_MASK;
	Clock_4kHz_CLKSTBY |= Clock_4kHz_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_4kHz_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_4kHz_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    Clock_4kHz_CLKEN &= (uint8)(~Clock_4kHz_CLKEN_MASK);
	Clock_4kHz_CLKSTBY &= (uint8)(~Clock_4kHz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_4kHz_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_4kHz_StopBlock(void) 
{
    if ((Clock_4kHz_CLKEN & Clock_4kHz_CLKEN_MASK) != 0u)
    {
...
#if defined(Clock_4kHz__CFG3)
        CLK_DIST_AMASK = Clock_4kHz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_4kHz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_4kHz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_4kHz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_4kHz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_4kHz_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_4kHz_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Clock_4kHz_CLKSTBY &= (uint8)(~Clock_4kHz_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Clock_4kHz_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_4kHz_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Clock_4kHz_GetSourceRegister();
    uint16 oldDivider = Clock_4kHz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_4kHz_CLKEN & Clock_4kHz_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_4kHz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_4kHz_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 Clock_4kHz_GetDividerRegister(void) 
{
    return CY_GET_REG16(Clock_4kHz_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_4kHz_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_4kHz_SetModeRegister(uint8 modeBitMask) 
{
    Clock_4kHz_MOD_SRC |= modeBitMask & (uint8)Clock_4kHz_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_4kHz_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_4kHz_ClearModeRegister(uint8 modeBitMask) 
{
    Clock_4kHz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_4kHz_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_4kHz_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_4kHz_GetModeRegister(void) 
{
    return Clock_4kHz_MOD_SRC & (uint8)(Clock_4kHz_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_4kHz_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_4kHz_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Clock_4kHz_GetDividerRegister();
    uint8 oldSrc = Clock_4kHz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_4kHz_MOD_SRC |= CYCLK_SSS;
        Clock_4kHz_MOD_SRC =
            (Clock_4kHz_MOD_SRC & (uint8)(~Clock_4kHz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_4kHz_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_4kHz_GetSourceRegister(void) 
{
    return Clock_4kHz_MOD_SRC & Clock_4kHz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_4kHz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_4kHz_SetPhaseRegister(uint8 clkPhase) 
{
    Clock_4kHz_PHASE = clkPhase & Clock_4kHz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_4kHz_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_4kHz_GetPhaseRegister(void) 
{
    return Clock_4kHz_PHASE & Clock_4kHz_PHASE_MASK;
}
